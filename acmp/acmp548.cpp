...
void solve() {
    ...
    while (i < len(a) && j < len(b))
        if (len(a) - i > 0 && len(b) - j > 0 && a.substr(i, len(a) - i) < b.substr(j, len(b) - j)) {
            if (a[i] != 'z') cout << a[i];
            ++i;
        } else {
            if (b[j] != 'z') cout << b[j];
...
#ifndef ONLINE_JUDGE
    //#define FILE_NAME "C:\\Users\\Кирилл\\Documents\\CodeBlocks\\in.txt"
    assert(freopen("C:\\Users\\Кирилл\\Documents\\CodeBlocks\\in.txt", "r", stdin));

//    asserr(freopen(FILE_NAME".txt", "w", stdout));
#endif
    while (read()) {
        solve();
        cout.flush();
