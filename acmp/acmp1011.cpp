...
int main() {
    ...
    vect AB(A, B), BC(B, C), AC(A, C);
    ld S = abs(kosoe(AB, AC)) / 2;
    ld a = AB.len, b = BC.len, c = AC.len;
    if (typeCircle == "In") //Xb = (a * Xa + b * Xb + C * Xc) / (a + b + c)
    {
        ld Xb = (a * A.x + b * B.x + c * C.x) / (a + b + c);
        ld Yb = (a * A.y + b * B.y + c * C.y) / (a + b + c);
        ld r = S/((a + b + c) / 2);
