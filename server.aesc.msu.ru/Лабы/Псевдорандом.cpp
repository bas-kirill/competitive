...
bool read() {
    start.resize(3);
    cin >> a >> b >> c >> d >> start[0] >> start[1] >> start[2];
    return 1;
}

vector<int> next(vector<int> x) {
    vector<int> res(3);
    res[0] = x[1];
    res[1] = x[2];
...
void solve() {
    vector<int> ptr1 = start;
    vector<int> ptr2 = next(start);
    ...
    ptr1 = start;
    ptr2 = start;
    for (int i = 0; i < len_period; ++i) {
        ptr2 = next(ptr2);
    }

    int len_pred_period = 0;
    while (ptr1 != ptr2) {
        ptr1 = next(ptr1);
        ptr2 = next(ptr2);
        ++len_pred_period;
    ...
    //pr(len_period, len_pred_period, good);
    cout << len_period << " " << len_pred_period << " " << good;
    //cout « fixed « setprecision(2) « len_period « " " « len_pred_period « " " « good « endl;
}
